...
const getSPAEntry = () => {
  return {
    index: config.disableHMR
      ? config.entry()
      : [config.entry(), 'webpack-hot-middleware/client.js?noInfo=true&reload=true']
  };
};
...
const getEntries = () => {
  ...
  for (let i = 0; i < entryFiles.length; i++) {
    const filename = entryFiles[i].substring(entryFiles[i].indexOf('views/') + 6, entryFiles[i].lastIndexOf('.'));
    map[filename] = config.disableHMR
      ? entryFiles[i]
      : [entryFiles[i], 'webpack-hot-middleware/client.js?noInfo=true&reload=true'];
  }
